...
if [[ "$MODE" == "bundled" ]]; then
  ...
  echo "   npm run base44:login-pack    (18 files — Sign In first)"
  echo "   npm run base44:export-pack   (79 files — full gallery)"
  echo "   npm run base44:publish-steps (browser instructions)"
  echo " For Mac-only UI (experimental): npm run fix:no-change -- --bundled"
fi
echo ""
echo " If phone still shows no change after fix:no-change:"
echo "   export-pack alone does NOT publish — open Base44 browser → Paste → Publish"
echo "   then: npm run prove:live-publish"
echo "══════════════════════════════════════════════════════════════"
