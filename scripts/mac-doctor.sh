...
echo ""
echo "=== Recommended sequence (1.0.1) ==="
echo "  1. bash scripts/mac-sync-github.sh"
echo "  2. bash scripts/mac-build.sh --no-git"
echo "  3. Delete app from iPhone → Xcode Clean → Run → Archive"
echo "  4. bash scripts/verify-xcode-app-bundle.sh"
echo ""
echo "  node scripts/verify-restorebraine-sync.mjs  (check git + features)"
echo ""
echo "Three copies:"
echo "  GitHub     — source (mac-sync-github.sh)"
echo "  iPhone     — bundled full app (mac-build.sh) — Base44 NOT required"
echo "  Base44 web — optional (only for --hosted Omega shell)"
echo ""
