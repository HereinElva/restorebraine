...
echo "  1. bash scripts/mac-sync-github.sh     (Mac = GitHub)"
echo "  2. bash scripts/mac-build.sh --no-git    (full Xcode replace)"
echo "  3. Xcode: Clean Build Folder -> Run (Cmd+R) -> Archive"
echo "  4. bash scripts/verify-xcode-app-bundle.sh"
echo ""
echo "Three copies (often confused):"
echo "  GitHub     — source code (HereinElva/restorebraine)"
echo "  Your Mac   — must match GitHub (mac-sync-github.sh)"
echo "  Base44 web — separate; NOT needed for bundled iPhone build"
echo ""
