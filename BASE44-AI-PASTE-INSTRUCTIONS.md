# Base44 AI — Full Copy-Paste Instructions (deploy v162)
...
- View source on https://restorebraine.com → `<meta name="restorebraine-deploy" content="v162">`
...
Restorebraine Base44 Publish Pack — deploy v162
...
╔════════════════════════════════════════════════════════════╗
║  PART 1 — Boot + auth (paste first)                        ║
╚════════════════════════════════════════════════════════════╝

════════════════════════════════════════════════════════════
BASE44 PATH: index.html
...
<html lang="en">
  <head>
    ...
    <meta name="restorebraine-deploy" content="v162" />
    <title>Restorebraine</title>
    <script src="/native-oauth-return.js"></script>
    <script>
(function(){var host=location.hostname,proto=location.protocol;if(proto==='capacitor:'||host==='localhost'||host==='127.0.0.1')return;var APP_ID='68fdc5f42768c4d045fe1bac',PLATFORM='https://app.base44.com',APP_ORIGIN=location.protocol+'//'+location.host,path=location.pathname.replace(/\/$/,'')||'/';function googleOAuth(fromUrl){var params=new URLSearchParams({app_id:APP_ID,from_url:fromUrl||'https://restorebraine.com',prompt:'select_account'});location.replace(PLATFORM+'/api/apps/auth/login?'+params.toString());}if(host==='base44.app'&&path==='/login'){googleOAuth('https://restorebraine.com');return;}if((host==='restorebraine.com'||host==='www.restorebraine.com')&&path==='/login'){googleOAuth(APP_ORIGIN);return;}var originalAssign=Location.prototype.assign,originalReplace=Location.prototype.replace;function guard(url){var value=String(url||'');if(/^https:\/\/base44\.app\/login/i.test(value)){googleOAuth('https://restorebraine.com');return true;}if(/^https:\/\/restorebraine\.com\/login/i.test(value)||/^https:\/\/www\.restorebraine\.com\/login/i.test(value)){googleOAuth('https://restorebraine.com');return true;}if(/^https:\/\/app\.base44\.com\/login/i.test(value)){googleOAuth('https://restorebraine.com');return true;}return false;}Location.prototype.assign=function(url){if(guard(url))return;return originalAssign.call(this,url);};Location.prototype.replace=function(url){if(guard(url))return;return originalReplace.call(this,url);};})();
    </script>
  ...
  <body>
    ...
    <div id="root">
      <div style="position:fixed;inset:0;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#eff6ff,#f5f3ff,#fdf2f8);font-family:system-ui,sans-serif;color:#374151;font-size:15px;font-weight:500;">Loading Restorebraine…</div>
    </div>
...
════════════════════════════════════════════════════════════
BASE44 PATH: public/native-oauth-return.js
ACTION: Open this exact path in the Base44 Code editor → Select All → Paste → Save
════════════════════════════════════════════════════════════
...
════════════════════════════════════════════════════════════
BASE44 PATH: public/login-redirect.js
ACTION: Open this exact path in the Base44 Code editor → Select All → Paste → Save
════════════════════════════════════════════════════════════
...
════════════════════════════════════════════════════════════
BASE44 PATH: src/deploy-marker.js
ACTION: Open this exact path in the Base44 Code editor → Select All → Paste → Save
════════════════════════════════════════════════════════════
// Base44: update these files in the Code editor, then click Publish (GitHub alone is not enough).
export const DEPLOY_BUILD = 162;



════════════════════════════════════════════════════════════
BASE44 PATH: src/lib/app-params.js
ACTION: Open this exact path in the Base44 Code editor → Select All → Paste → Save
════════════════════════════════════════════════════════════
/**
 * app-params.js
...
════════════════════════════════════════════════════════════
BASE44 PATH: src/lib/app-domains.js
ACTION: Open this exact path in the Base44 Code editor → Select All → Paste → Save
════════════════════════════════════════════════════════════
import { LOCAL_NATIVE_BUNDLE } from '@/lib/native-bundle-mode';
...
════════════════════════════════════════════════════════════
BASE44 PATH: src/lib/auth-urls.js
ACTION: Open this exact path in the Base44 Code editor → Select All → Paste → Save
════════════════════════════════════════════════════════════
...
════════════════════════════════════════════════════════════
BASE44 PATH: src/lib/sign-in-with-google.js
ACTION: Open this exact path in the Base44 Code editor → Select All → Paste → Save
════════════════════════════════════════════════════════════
...
════════════════════════════════════════════════════════════
BASE44 PATH: src/lib/AuthContext.jsx
ACTION: Open this exact path in the Base44 Code editor → Select All → Paste → Save
════════════════════════════════════════════════════════════
...
════════════════════════════════════════════════════════════
BASE44 PATH: src/screens/SignInScreen.jsx
ACTION: Open this exact path in the Base44 Code editor → Select All → Paste → Save
════════════════════════════════════════════════════════════
...
════════════════════════════════════════════════════════════
BASE44 PATH: src/screens/sign-in.css
ACTION: Open this exact path in the Base44 Code editor → Select All → Paste → Save
════════════════════════════════════════════════════════════
.rb-signin {
  position: fixed;
...
════════════════════════════════════════════════════════════
BASE44 PATH: src/lib/login-logo-data.js
ACTION: Open this exact path in the Base44 Code editor → Select All → Paste → Save
════════════════════════════════════════════════════════════
/** Generated by scripts/embed-login-logo.mjs — do not edit */
export const LOGIN_LOGO_DATA_URL = 'data:image/png;base64,';



════════════════════════════════════════════════════════════
BASE44 PATH: src/lib/native-hosted-redirect.js
ACTION: Open this exact path in the Base44 Code editor → Select All → Paste → Save
════════════════════════════════════════════════════════════
...
╔════════════════════════════════════════════════════════════╗
║  PART 3 — Gallery UI + Layout + CSS (paste last, then Publish)║
╚════════════════════════════════════════════════════════════╝

════════════════════════════════════════════════════════════
BASE44 PATH: src/lib/native-bundle-mode.js
ACTION: Open this exact path in the Base44 Code editor → Select All → Paste → Save
════════════════════════════════════════════════════════════
// Base44 hosted web — must be false (iOS v4 native uses true via build:native-local)
export const LOCAL_NATIVE_BUNDLE = false;



════════════════════════════════════════════════════════════
BASE44 PATH: src/lib/native-oauth-fix.js
ACTION: Open this exact path in the Base44 Code editor → Select All → Paste → Save
════════════════════════════════════════════════════════════
...
════════════════════════════════════════════════════════════
BASE44 PATH: src/lib/native-platform-guard.js
ACTION: Open this exact path in the Base44 Code editor → Select All → Paste → Save
════════════════════════════════════════════════════════════
...
════════════════════════════════════════════════════════════
BASE44 PATH: src/lib/native-google-oauth.js
ACTION: Open this exact path in the Base44 Code editor → Select All → Paste → Save
════════════════════════════════════════════════════════════
...
AFTER PUBLISH — verify:
  ...
  • Must show: <meta name="restorebraine-deploy" content="v162">
